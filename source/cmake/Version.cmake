 ...
 #
 #################################################################################################################
 # PURPOSE: Identity version control software version display, also read version files to present s265 version.
 #################################################################################################################
 #Default Settings, for user to be vigilant about s265 version being reported during product build.
set(S265_VERSION "unknown")
set(S265_LATEST_TAG "0.0")
set(S265_TAG_DISTANCE "0")
...
if(GIT_FOUND)
    ...
    if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/../.git)
        set(GIT_ARCHETYPE "0")
    elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/../s265Version.txt)
        set(GIT_ARCHETYPE "1")
    endif()
endif(GIT_FOUND)
if(HG_ARCHETYPE)
    #Read the lines of the archive summary file to extract the version
    message(STATUS "SOURCE CODE IS FROM s265 HG ARCHIVED ZIP OR TAR BALL")
    ...
    endforeach()
    if(DEFINED hg_tag)
        set(S265_LATEST_TAG ${hg_tag})
    elseif(DEFINED hg_node)
        set(S265_LATEST_TAG ${hg_latesttag})
        set(S265_TAG_DISTANCE ${hg_latesttagdistance})
        string(SUBSTRING "${hg_node}" 0 12 S265_REVISION_ID)
    endif()
...
# means that's its neither hg archive nor git clone/archive hence it has to be hg live repo as these are only four cases that need to processed in mutual exclusion.
    execute_process(COMMAND
        ...
        OUTPUT_VARIABLE S265_LATEST_TAG
        ERROR_QUIET
    ...
    execute_process(COMMAND
        ...
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        OUTPUT_VARIABLE S265_TAG_DISTANCE
        ERROR_QUIET
        OUTPUT_STRIP_TRAILING_WHITESPACE
        )
    execute_process(
        ...
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        OUTPUT_VARIABLE S265_REVISION_ID
        ERROR_QUIET
        OUTPUT_STRIP_TRAILING_WHITESPACE
        )
    string(SUBSTRING "${S265_REVISION_ID}" 0 12 S265_REVISION_ID)
    if(S265_LATEST_TAG MATCHES "^r")
        string(SUBSTRING ${S265_LATEST_TAG} 1 -1 S265_LATEST_TAG)
    endif()
    message(STATUS "HG LIVE REPO STATUS CHECK DONE")
elseif(GIT_ARCHETYPE)
    message(STATUS "SOURCE CODE IS FROM s265 GIT ARCHIVED ZIP OR TAR BALL")
    #Read the lines of the archive summary file to extract the version
    file(READ ${CMAKE_CURRENT_SOURCE_DIR}/../s265Version.txt filebuf)
    STRING(REGEX REPLACE "\n" ";" filebuf "${filebuf}")
    ...
    if(DEFINED git_releasetag)
        set(S265_LATEST_TAG ${git_releasetag})
        if(DEFINED git_releasetagdistance)
            set(S265_TAG_DISTANCE ${git_releasetagdistance})
            if(S265_TAG_DISTANCE STRGREATER "0" OR S265_TAG_DISTANCE STREQUAL "0")
                #for s265 the repository changeset has to be a tag id or commit id after the tag
                #hence mandating it's presence in version file always for valid tag distances.
                if(DEFINED git_repositorychangeset)
                    string(SUBSTRING "${git_repositorychangeset}" 0 9 S265_REVISION_ID)
                else()
                    message(WARNING "S265 LATEST COMMIT TIP INFORMATION NOT AVAILABLE")
                endif()
            else()
                message(WARNING "S265 TAG DISTANCE INVALID")
            endif()
        else()
            message(WARNING "COMMIT INFORMATION AFTER LATEST REVISION UNAVAILABLE")
        endif()
    else()
        message(WARNING "S265 RELEASE VERSION LABEL MISSING: ${S265_LATEST_TAG}")
    endif()
    ...
    execute_process(
        COMMAND
        ${GIT_EXECUTABLE} rev-list ${S265_LATEST_TAG}.. --count --first-parent
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        OUTPUT_VARIABLE S265_TAG_DISTANCE
        ERROR_QUIET
        OUTPUT_STRIP_TRAILING_WHITESPACE
...
if(S265_TAG_DISTANCE STREQUAL "0")
    set(S265_VERSION "${S265_LATEST_TAG}")
elseif(S265_TAG_DISTANCE STRGREATER "0")
    set(S265_VERSION "${S265_LATEST_TAG}+${S265_TAG_DISTANCE}-${S265_REVISION_ID}")
endif()

#will always be printed in its entirety based on version file configuration to avail revision monitoring by repo owners
message(STATUS "S265 RELEASE VERSION ${S265_VERSION}")
